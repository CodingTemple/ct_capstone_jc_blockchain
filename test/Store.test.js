...
contract('ComicStore', ([storeOwner, buyer, seller]) => {
  const TaxPercent = 10
  const storeName = 'ComicStore'

  let store

  beforeEach(async () => {
    store = await Store.new(storeName, storeOwner, TaxPercent)
  })
  ...
  describe('deployment', () => {
    ...
    it('confirms sales fee', async () => {
      const result = await store.TaxPercent()
      result.toString().should.equal(TaxPercent.toString())
    })
  })
